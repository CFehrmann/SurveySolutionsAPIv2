...
- workspace can now be set with *suso_set_key*
- *suso_export* allows to merge all the data export files into one
- Spatial data (i.e. polygons, point locations, lines) can be processed
  into sf objects
...
Solutions REST API, which one can easily address through using i.e. the
excellent httr package which also constitutes the basis for this
package. The main intention of this package though, is to be integrated
into your own data collection workflow, either through a shiny
application or just through a simple script. This allows you to build
your own workflows with your own customized user interfaces for your
Survey Solutions data collection operations, no matter if you are
...
details. After this done, you can use the *suso_set_key()* function, to
...
Its return value is a list with the following elements: main, R1, R2,
R3, with

- **main** containing the files: interview\_\_comments, kasai_listing1
...
To upload a map, you can just use the **suso_mapupload** function like
this:
